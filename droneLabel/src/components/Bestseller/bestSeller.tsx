  ...
  return (
    ...
    >
      ...
      <div className="pt-2.5 sm:pt-3 px-0.5">
        <p className="font-semibold text-[#1a1a1a] text-xs sm:text-sm leading-tight truncate" style={{ fontFamily: "'Barlow', sans-serif" }}>{name}</p>
        <p className="text-[#999] text-xs mt-0.5 mb-1.5 truncate" style={{ fontFamily: "'Barlow', sans-serif" }}>{category}</p>
        <p className="font-semibold text-[#1a1a1a] text-xs sm:text-sm" style={{ fontFamily: "'Barlow', sans-serif" }}>{price}</p>
      </div>
    </div>
  );
};
...
const ProductRow = ({ title, items }) => {
  ...
  const animateTo = (targetLeft, duration = 650) => {
    ...
    <section className="w-full mb-12 sm:mb-14 last:mb-0">
      ...
      <div className="text-center mb-5 sm:mb-7 px-4">
        <h2
          ...
          style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "clamp(24px, 4vw, 42px)", letterSpacing: "-0.01em" }}
        >
          {title}
        </h2>
      </div>
